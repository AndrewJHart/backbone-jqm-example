...
(function ($) {
    ...
    window.EventDetailView = Backbone.View.extend({
        ...
        deleteEvent:function () {
            ...
            this.model.destroy({
                ...
                error: function() {
                    window.console.log('no access to server - heres where we need to use localstorage');
                    App.navigate("", true);
                }
            });
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        initialize: function () {
            ...
            this.firstPage = true;
            this.refListView = false;
            this.refDetailView = false;
            this.refAddView = false;
        ...
        list: function () {
            ...
            this.refListView = new MessagePageView({model: this.messages, collection: this.messages });
            this.changePage(this.refListView);
        },
        ...
        detail:function (id) {
            ...
            if (this.refDetailView)
                this.refDetailView.close();

            // get our single model from the collection by id
            if (App.messages) {
                this.message = App.messages.get(id);
                this.refDetailView = new EventDetailView({ model: this.message });
                this.changePage(this.refDetailView);
            }
            else {
        ...
        add: function() {
            ...
            if (this.refAddView)
                this.refAddView.close();

            // render a new view for posting messages
            this.refAddView = new MessageCreateView({model: new Message(), collection: this.messages });
            this.changePage(this.refAddView);  // pass collection by reference
        ...
        changePage: function (page) {
            ...
            console.log('successfully called $.mobile.changePage on jqm');
        }
    });
